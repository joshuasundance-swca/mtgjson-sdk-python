...
class PriceQuery:
    ...
    Prices come from AllPricesToday.parquet and AllPrices.parquet,
    registered as DuckDB views on first access.
    ...
    def get(self, uuid: str) -> dict | None:
        ...
        rows = self._conn.execute(
            "SELECT * FROM all_prices_today WHERE uuid = $1 "
            "ORDER BY source, provider, price_type, finish, date",
            [uuid],
        )
        if not rows:
            return None
        ...
        for r in rows:
            ...
            cat = prov.setdefault(r["price_type"], {})
    ...
    def today(
        ...
        *,
        ...
        if price_type:
            parts.append(f"AND price_type = ${idx}")
    ...
    def history(
        ...
        *,
        ...
        """
        self._conn.ensure_views("all_prices")
        parts = ["SELECT * FROM all_prices WHERE uuid = $1"]
        params: list[Any] = [uuid]
        idx = 2
    ...
    def price_trend(
        ...
        *,
        ...
        params: list[Any] = [uuid, price_type]
        idx = 3
    ...
    def cheapest_printing(
        ...
        *,
        ...
        rows = self._conn.execute(sql, [name, provider, finish, price_type])
    ...
    def cheapest_printings(
        ...
        *,
        ...
        return self._conn.execute(sql, [provider, finish, price_type])
